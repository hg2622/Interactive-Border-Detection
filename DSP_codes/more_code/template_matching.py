...
def template_match_with_threshold(template, target, threshold_value, w, h, max_boxes):
    ...
    Parameters:
        ...
        threshold_value (float): Maximum acceptable error for matches.
...
def template_match_multiple_templates(template_paths, target_path, scale_factor=1, threshold_ratio=0.1, max_boxes=10):
    """
    ...
    for template_path in template_paths:
        ...
        for channel in range(3):
            result = cv2.matchTemplate(target[:, :, channel], resized_template[:, :, channel], cv2.TM_SQDIFF)
            result_bgr.append(result)

        combined_result = sum(result_bgr)
        min_val, _, min_loc, _ = cv2.minMaxLoc(combined_result)
...
result_image = template_match_multiple_templates(template_paths, target_path, scale_factor=0.2, threshold_ratio=0.2,
                                                 max_boxes=10)
